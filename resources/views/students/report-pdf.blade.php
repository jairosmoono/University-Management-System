...
<table class="hdr">
    <tr>
        ...
        <td style="text-align:right">
            ...
            <div class="doc-meta">Total Records: <strong>{{ $students->count() }}</strong></div>
        </td>
    </tr>
</table>
...
<div class="filters">
    ...
    @foreach($filters as $label => $value)
        {{ $label }}: <strong>{{ $value }}</strong>@if(!$loop->last) &nbsp;&bull;&nbsp; @endif
    @endforeach
</div>
@endif
...
<table class="sum">
    <tr>
        ...
        <td class="t-green">
            <div class="s-val">{{ $activeCount }}</div>
            <div class="s-lbl">Active</div>
        </td>
        <td class="t-red">
            <div class="s-val">{{ $dropoutCount }}</div>
            <div class="s-lbl">Dropped Out</div>
        </td>
        <td class="t-info">
            <div class="s-val">{{ $maleCount }}</div>
            <div class="s-lbl">Male</div>
        </td>
        <td class="t-warn">
            <div class="s-val">{{ $femaleCount }}</div>
...
<table class="data">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="font-weight:600">{{ optional($s->user)->name ?? '—' }}</td>
            <td style="font-size:7px">{{ optional($s->user)->email ?? '—' }}</td>
            <td>{{ $s->phone ?: '—' }}</td>
            <td>{{ ucfirst($s->gender ?? '—') }}</td>
            <td>{{ $s->national_id ?: '—' }}</td>
            <td>{{ optional($s->program)->name ?? '—' }}</td>
            ...
            <td>
                ...
                @endphp
                <span class="badge {{ $bc }}">{{ $s->status === 'dropped_out' ? 'Dropped Out' : ucfirst($s->status) }}</span>
            </td>
        </tr>
        @empty
        <tr>
...
<table class="foot">
    <tr>
        <td>Confidential &mdash; {{ $uniName }}</td>
        <td class="r">Student Report &mdash; {{ now()->format('d M Y') }} &mdash; Page 1</td>
    </tr>
</table>
