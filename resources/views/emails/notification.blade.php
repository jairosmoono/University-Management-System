...
<div class="wrapper">
  ...
  <div class="badge-wrap">
    <span class="type-badge" style="{{ $badgeStyle }}">
      {{ $typeLabels[$notifType] ?? 'Notification' }}
    </span>
  </div>
  ...
  <div class="body">
    ...
    @endif
    <div class="message">{!! nl2br(e($notifMessage)) !!}</div>

    @if($actionUrl)
    <div class="cta-wrap">
      <a href="{{ $actionUrl }}" class="cta">{{ $actionLabel }}</a>
    </div>
    @endif
    ...
    <div style="margin-top:24px;padding:14px 18px;background:#f8f9fa;border-radius:6px;border:1px solid #e9ecef">
      <p style="font-size:13px;font-weight:600;color:#0B1F3A;margin:0 0 8px">
        <span style="margin-right:4px">&#128206;</span> Attachments ({{ count($attachmentPaths) }})
      </p>
  ...
  <div class="footer">
    <p>
      ...
      To manage your email preferences, <a href="{{ url('/notifications/preferences') }}">click here</a>.
    </p>
