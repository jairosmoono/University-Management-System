...
<head>
    ...
    <title>@yield('title', 'Login') - {{ $__uni['university_name'] ?? config('app.name', 'University Management System') }}</title>
    @if(!empty($__uni['favicon_path']))
    <link rel="icon" href="{{ asset('storage/' . $__uni['favicon_path']) }}" type="image/x-icon">
    @endif
                ...
                <div class="row g-0">
                    <div class="col-md-5 auth-left">
                        <div>
                            ...
                            @endif
                            <h1>{{ $__uni['university_name'] ?? config('app.name', 'University Management System') }}</h1>
                            <p>A comprehensive platform for managing all aspects of university operations.</p>
                            <div class="mt-4">
                                @foreach([['bi-mortarboard', 'Academic Management'], ['bi-people', 'Student Services'], ['bi-cash-stack', 'Finance & Billing'], ['bi-bar-chart', 'Analytics & Reports']] as [$icon, $label])
