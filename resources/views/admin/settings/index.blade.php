...
<div class="tab-content">
    ...
    <div class="tab-pane fade show active" id="general">
        ...
        {{-- Branding: separate form (needs multipart) --}}
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-transparent border-0 py-3">
                <h6 class="mb-0 fw-semibold">Branding — Logo & Favicon</h6>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('admin.settings.upload-branding') }}" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="d-flex align-items-center gap-3 mb-2">
                                @if(!empty($settings['logo_path']))
                                    <img src="{{ asset('storage/' . $settings['logo_path']) }}" alt="Logo" class="border rounded p-1" style="height:64px;max-width:180px;object-fit:contain;">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="d-flex align-items-center gap-3 mb-2">
                                @if(!empty($settings['favicon_path']))
                                    <img src="{{ asset('storage/' . $settings['favicon_path']) }}" alt="Favicon" class="border rounded p-1" style="height:40px;width:40px;object-fit:contain;">
                                    <span class="badge bg-success">Uploaded</span>
                                @else
                    ...
                    <div class="mt-3">
                        ...
                        @if(!empty($settings['logo_path']) || !empty($settings['favicon_path']))
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body">
                ...
                {{-- Current images --}}
                @php $heroImages = $settings['hero_images'] ?? []; @endphp
                @if(count($heroImages))
                <div class="row g-3 mb-4" id="heroImageGrid">
                    ...
                    <div class="col-6 col-md-4 col-lg-3 hero-img-item">
                        <div class="position-relative rounded overflow-hidden border" style="aspect-ratio:16/9;background:#000">
                            <img src="{{ asset('storage/' . $img) }}" alt="Hero slide"
                                 style="width:100%;height:100%;object-fit:cover;opacity:.9">
                            <form method="POST" action="{{ route('admin.settings.hero-images.delete') }}"
                ...
                {{-- Upload form --}}
                <form method="POST" action="{{ route('admin.settings.hero-images.upload') }}" enctype="multipart/form-data">
                    @csrf
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-8">
                            ...
                            <input type="text" name="university_name" class="form-control" value="{{ $settings['university_name'] ?? config('university.name') }}" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Short Name / Abbreviation</label>
                            <input type="text" name="university_short_name" class="form-control" value="{{ $settings['university_short_name'] ?? '' }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Email Address</label>
                            <input type="email" name="university_email" class="form-control" value="{{ $settings['university_email'] ?? config('university.email') }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Phone Number</label>
                            <input type="text" name="university_phone" class="form-control" value="{{ $settings['university_phone'] ?? config('university.phone') }}">
                        </div>
                        <div class="col-12">
                            <label class="form-label">Address</label>
                            <textarea name="university_address" class="form-control" rows="2">{{ $settings['university_address'] ?? config('university.address') }}</textarea>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">City</label>
                            <input type="text" name="university_city" class="form-control" value="{{ $settings['university_city'] ?? '' }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Country</label>
                            <input type="text" name="university_country" class="form-control" value="{{ $settings['university_country'] ?? 'Zambia' }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Website</label>
                            <input type="url" name="university_website" class="form-control" value="{{ $settings['university_website'] ?? '' }}">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" name="currency_symbol" class="form-control" value="{{ $settings['currency_symbol'] ?? 'K' }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Currency Code</label>
                            <input type="text" name="currency_code" class="form-control" value="{{ $settings['currency_code'] ?? 'ZMW' }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Library Fine per Day</label>
                            <div class="input-group">
                                <span class="input-group-text">K</span>
                                <input type="number" name="library_fine_rate" class="form-control" value="{{ $settings['library_fine_rate'] ?? '5' }}" min="0" step="0.5">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="maintenance_mode" id="maintenanceMode" value="1" {{ ($settings['maintenance_mode'] ?? false) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="registration_open" id="regOpen" value="1" {{ ($settings['registration_open'] ?? true) ? 'checked' : '' }}>
                                <label class="form-check-label" for="regOpen">Student Registration Open</label>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="admissions_open" id="admissionsOpen" value="1" {{ ($settings['admissions_open'] ?? true) ? 'checked' : '' }}>
    ...
    <div class="tab-pane fade" id="academic-settings">
            ...
            <div class="col-md-6">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-header bg-transparent border-0 py-3 d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('academic.academic-years.index') }}" class="btn btn-sm btn-outline-primary">Manage All</a>
                    </div>
                    <div class="card-body">
                        @if($currentAcademicYear)
                            <div class="d-flex align-items-center gap-3 mb-3">
                                ...
                                <div>
                                    <h5 class="mb-0 fw-bold">{{ $currentAcademicYear->name }}</h5>
                                    <small class="text-muted">{{ $currentAcademicYear->start_date->format('d M Y') }} – {{ $currentAcademicYear->end_date->format('d M Y') }}</small>
                                </div>
                            </div>
                            <span class="badge bg-success">Active</span>
                            <span class="badge bg-light text-dark ms-1">{{ $currentAcademicYear->semesters->count() }} semester(s)/term(s)</span>
                        @else
            ...
            <div class="col-md-6">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-header bg-transparent border-0 py-3 d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('academic.semesters.index') }}" class="btn btn-sm btn-outline-primary">Manage All</a>
                    </div>
                    <div class="card-body">
                        @if($currentSemester)
                            <div class="d-flex align-items-center gap-3 mb-3">
                                ...
                                <div>
                                    <h5 class="mb-0 fw-bold">{{ $currentSemester->name }}</h5>
                                    <small class="text-muted">{{ optional($currentSemester->academicYear)->name }}</small>
                                </div>
                            </div>
                            <div class="row g-2 text-center mb-2">
                                <div class="col-6">
                                    <small class="text-muted d-block">Start</small>
                                    <span class="fw-semibold small">{{ $currentSemester->start_date->format('d M Y') }}</span>
                                </div>
                                <div class="col-6">
                                    <small class="text-muted d-block">End</small>
                                    <span class="fw-semibold small">{{ $currentSemester->end_date->format('d M Y') }}</span>
                                </div>
                            </div>
                            @php
                                $statusColors = ['upcoming'=>'secondary','registration'=>'info','active'=>'success','exam'=>'warning','completed'=>'dark'];
                            @endphp
                            <span class="badge bg-{{ $statusColors[$currentSemester->status] ?? 'secondary' }}">{{ ucfirst($currentSemester->status) }}</span>
            ...
            <div class="card-body p-0">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ $ay->is_current ? 'table-success' : '' }}">
                            <td class="fw-semibold">
                                {{ $ay->name }}
                                @if($ay->is_current)<span class="badge bg-success ms-1">Current</span>@endif
                            </td>
                            <td><small>{{ $ay->start_date->format('M Y') }} – {{ $ay->end_date->format('M Y') }}</small></td>
                            <td>
                                @foreach($ay->semesters as $sem)
                                    <span class="badge bg-{{ $statusColors[$sem->status] ?? 'secondary' }} me-1">
                                        {{ $sem->name }}{{ $sem->is_current ? ' ★' : '' }}
                                    </span>
                                @endforeach
                                @if($ay->semesters->isEmpty())<span class="text-muted small">none</span>@endif
                            </td>
                            <td>
                                @php $ayColors = ['upcoming'=>'secondary','active'=>'success','completed'=>'dark']; @endphp
                                <span class="badge bg-{{ $ayColors[$ay->status] ?? 'secondary' }}">{{ ucfirst($ay->status) }}</span>
                            </td>
                            <td>
                                @if(!$ay->is_current)
                                <form method="POST" action="{{ route('academic.academic-years.set-current', $ay) }}" class="d-inline">
                                    @csrf
                                ...
                                @endif
                                <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#addSemesterModal{{ $ay->id }}" title="Add Semester/Term">
                                    <i class="bi bi-plus"></i> Semester/Term
                                </button>
                                <a href="{{ route('academic.academic-years.edit', $ay) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></a>
                            </td>
                        </tr>
                        @empty
            ...
            <div class="card-body">
                <div class="d-flex flex-wrap gap-2 mb-3">
                    ...
                    <span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 d-inline-flex align-items-center gap-1 px-3 py-2 fs-6">
                        {{ ucfirst($ct) }}
                        <form method="POST" action="{{ route('admin.settings.course-types.destroy', $ct) }}" class="d-inline" onsubmit="return confirm('Remove course type \'{{ $ct }}\'?')">
                            @csrf @method('DELETE')
                ...
                @if(session('success') && str_contains(session('success'), 'Course type'))
                <div class="alert alert-success py-2 small">{{ session('success') }}</div>
                @endif
                <form method="POST" action="{{ route('admin.settings.course-types.store') }}" class="d-flex gap-2 align-items-start" style="max-width:380px">
                    @csrf
                    <div class="flex-grow-1">
                        <input type="text" name="course_type" class="form-control form-control-sm @error('course_type') is-invalid @enderror"
        ...
        {{-- Semesters/Terms across all years --}}
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        @foreach($academicYears as $ay)
                            @foreach($ay->semesters->sortBy('start_date') as $sem)
                            <tr class="{{ $sem->is_current ? 'table-warning' : '' }}">
                                <td class="fw-semibold">
                                    {{ $sem->name }}
                                    @if($sem->is_current)<span class="badge bg-warning text-dark ms-1">Current</span>@endif
                                </td>
                                <td><small>{{ $ay->name }}</small></td>
                                <td><small>{{ $sem->start_date->format('d M Y') }} – {{ $sem->end_date->format('d M Y') }}</small></td>
                                <td>
                                    @if($sem->registration_start && $sem->registration_end)
                                        <small>{{ $sem->registration_start->format('d M') }} – {{ $sem->registration_end->format('d M Y') }}</small>
                                    @else
                                        <small class="text-muted">—</small>
                                    @endif
                                </td>
                                <td><span class="badge bg-{{ $statusColors[$sem->status] ?? 'secondary' }}">{{ ucfirst($sem->status) }}</span></td>
                                <td>
                                    @if(!$sem->is_current)
                                    <form method="POST" action="{{ route('academic.semesters.set-current', $sem) }}" class="d-inline">
                                        @csrf
                                    ...
                                    @endif
                                    <a href="{{ route('academic.semesters.edit', $sem) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></a>
                                </td>
                            </tr>
                            @endforeach
                        @endforeach
                        @if($academicYears->every(fn($ay) => $ay->semesters->isEmpty()))
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                ...
                <div class="row g-3 mt-2">
                    <div class="col-md-4"><label class="form-label text-muted small">Driver</label><p class="fw-semibold">{{ config('mail.default') }}</p></div>
                    <div class="col-md-4"><label class="form-label text-muted small">Host</label><p class="fw-semibold">{{ config('mail.mailers.smtp.host', '—') }}</p></div>
                    <div class="col-md-4"><label class="form-label text-muted small">Port</label><p class="fw-semibold">{{ config('mail.mailers.smtp.port', '—') }}</p></div>
                    <div class="col-md-6"><label class="form-label text-muted small">From Address</label><p class="fw-semibold">{{ config('mail.from.address', '—') }}</p></div>
                    <div class="col-md-6"><label class="form-label text-muted small">From Name</label><p class="fw-semibold">{{ config('mail.from.name', '—') }}</p></div>
                </div>
            </div>
        </div>
    </div>
                ...
                <table class="table datatable table-hover table-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-{{ in_array($log->action, ['POST','PUT','PATCH']) ? 'warning' : ($log->action === 'DELETE' ? 'danger' : 'secondary') }}">{{ $log->action }}</span></td>
                            <td><small class="text-muted">{{ $log->model_type }}</small></td>
                            <td><small class="text-truncate" style="max-width:200px">{{ $log->url }}</small></td>
...
{{-- Add Semester/Term Modals (one per academic year) --}}
@foreach($academicYears as $ay)
<div class="modal fade" id="addSemesterModal{{ $ay->id }}" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <form method="POST" action="{{ route('academic.semesters.store') }}">
            @csrf
            <input type="hidden" name="academic_year_id" value="{{ $ay->id }}">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Add Semester/Term — {{ $ay->name }}</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-check mt-3">
                        <input class="form-check-input" type="checkbox" name="is_current" value="1" id="isCurrentSem{{ $ay->id }}">
                        <label class="form-check-label" for="isCurrentSem{{ $ay->id }}">Set as current semester/term</label>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
