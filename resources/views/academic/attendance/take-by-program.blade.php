...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted mb-0">
            <span class="badge bg-success me-1">{{ $program->code ?? $program->name }}</span>
            {{ $program->name }}
            &bull; <span class="fw-semibold">{{ optional($offering->course)->code }} — {{ optional($offering->course)->name }}</span>
            &bull; {{ \Carbon\Carbon::parse($today)->format('d M Y') }}
        </p>
    </div>
...
<form action="{{ route('academic.attendance.save') }}" method="POST">
    ...
    <div class="card border-0 shadow-sm mb-3">
        <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
            <h6 class="mb-0">Students in {{ $program->name }} ({{ $students->count() }})</h6>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr><th>#</th><th>Student</th><th>ID</th><th>Status</th><th>Remarks</th></tr>
                    </thead>
                    <tbody>
                        @forelse($students as $student)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ optional($student->user)->name ?: $student->student_id }}</td>
                            <td><code>{{ $student->student_id }}</code></td>
                            <td>
                                <div class="btn-group" role="group">
                                    @foreach(['present','absent','late','excused'] as $status)
                                    <input type="radio" class="btn-check" name="attendance[{{ $student->id }}]" value="{{ $status }}"
                                        id="att_{{ $student->id }}_{{ $status }}" {{ $loop->first ? 'checked' : '' }}>
                                    <label class="btn btn-sm btn-outline-{{ $status === 'present' ? 'success' : ($status === 'absent' ? 'danger' : ($status === 'late' ? 'warning' : 'info')) }}"
                                        for="att_{{ $student->id }}_{{ $status }}">{{ ucfirst($status) }}</label>
                                    @endforeach
                                </div>
                            </td>
                        ...
                        @empty
                        <tr><td colspan="5" class="text-center text-muted py-4">No active students found in this program.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
