    ...
    <div>
        ...
        <nav aria-label="breadcrumb"><ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active">
                {{ $isStudentView ? 'My Registrations' : ($isLecturerView ? 'My Students' : 'Registrations') }}
            </li>
        </ol></nav>
    </div>
    @can('manage-academic')
    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#registerModal" {{ !$registrationOpen && !auth()->user()->hasRole('super-admin') ? 'disabled' : '' }}>
        ...
        <div class="card border-0 shadow-sm stat-card">
            <div class="card-body d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <div class="stat-label text-muted">{{ $isStudentView ? 'Total Courses' : 'Total Registrations' }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card border-0 shadow-sm stat-card">
            <div class="card-body d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <div class="stat-label text-muted">{{ $isStudentView ? 'Active' : 'Students Registered' }}</div>
                </div>
            </div>
        </div>
    </div>
    <div class="col">
        <div class="card border-0 shadow-sm stat-card">
            <div class="card-body d-flex align-items-center gap-3">
                ...
                <div>
                    <div class="stat-value">{{ $stats['dropped'] ?? 0 }}</div>
...
<div class="card border-0 shadow-sm mb-3">
    <div class="card-body py-2">
        <form method="GET" class="row g-2 align-items-center">
            @if($isLecturerView)
            {{-- Lecturer: filter by their own course offerings --}}
            <div class="col-md-4">
                <select name="offering_id" class="form-select form-select-sm" onchange="this.form.submit()">
                    <option value="">All My Courses</option>
                    @foreach($offeringsForFilter as $o)
                    <option value="{{ $o->id }}" {{ request('offering_id') == $o->id ? 'selected' : '' }}>
                        {{ optional($o->course)->code }} — {{ Str::limit(optional($o->course)->title, 35) }}
                    </option>
                    @endforeach
                </select>
            </div>
            @elseif(!$isStudentView)
            {{-- Admin/Registrar: full filters --}}
            <div class="col-md-2">
            ...
            @else
            {{-- Student: semester/term filter only --}}
            <div class="col-md-3">
                <select name="semester_id" class="form-select form-select-sm" onchange="this.form.submit()">
                    <option value="">All Semesters/Terms</option>
                    @foreach($semesters as $s)
            ...
            @if(!$isStudentView)
            <div class="col-md-4">
                <input type="text" name="search" class="form-control form-control-sm" placeholder="Search by student ID or name..." value="{{ request('search') }}">
            </div>
            @endif
            <div class="col-auto">
                @if(!$isStudentView)
                ...
                @endif
                @if(request()->hasAny(['search','status','offering_id','semester_id','program_id']))
                <a href="{{ route('academic.registrations.index') }}" class="btn btn-sm btn-outline-secondary ms-1">Clear</a>
                @endif
            </div>
        </form>
    </div>
...
<div class="card border-0 shadow-sm">
    <div class="card-body p-0">
        ...
        @if(!$isStudentView && !$isLecturerView && $studentList)
        {{-- Admin view: one row per student --}}
                    ...
                    <td class="ps-3">
                        <div class="d-flex align-items-center gap-2">
                            <div class="rounded-circle bg-primary bg-opacity-10 text-primary d-flex align-items-center justify-content-center fw-bold flex-shrink-0"
                                ...
                                {{ strtoupper(substr(optional($student->user)->name ?? 'S', 0, 1)) }}
                            </div>
                            <div>
                                <div class="fw-semibold" style="font-size:0.85rem">{{ optional($student->user)->name ?? '—' }}</div>
                                <small class="text-muted">{{ $student->student_id }}</small>
                            </div>
                        </div>
                    </td>
                    <td><small class="text-muted">{{ optional($student->program)->name ?? '—' }}</small></td>
                    <td class="text-center"><span class="badge bg-primary">{{ $student->total_subjects }}</span></td>
                    <td class="text-center"><span class="badge bg-success">{{ $student->registered_count }}</span></td>
                    <td class="text-center"><span class="badge bg-danger">{{ $student->dropped_count }}</span></td>
                    <td class="text-end pe-3">
                        <div class="d-flex gap-1 justify-content-end">
                            <button class="btn btn-sm btn-outline-primary"
                                    onclick="viewSubjects({{ $student->id }}, '{{ addslashes(optional($student->user)->name) }}', '{{ $student->student_id }}')">
                                <i class="bi bi-eye me-1"></i>View
                            </button>
                            <button class="btn btn-sm btn-outline-secondary"
                                    onclick="editRegistrations({{ $student->id }}, '{{ addslashes(optional($student->user)->name) }}', '{{ $student->student_id }}')">
        ...
        @else
        {{-- Lecturer / Student view: one row per registration --}}
        <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
                <tr>
                    @if(!$isStudentView)
                    <th class="ps-3">Student</th>
                    @if($isLecturerView)<th>Programme</th>@endif
                    @endif
                    <th class="{{ $isStudentView ? 'ps-3' : '' }}">Course</th>
                    <th>Semester/Term</th>
                    @if(!$isLecturerView)<th>Lecturer</th>@endif
                    <th>Status</th>
                    <th>Registered On</th>
                    @can('manage-academic')<th class="text-end pe-3">Action</th>@endcan
                </tr>
            </thead>
            <tbody>
                @forelse($registrations as $reg)
                @php $colors = ['registered'=>'success','approved'=>'success','dropped'=>'danger','completed'=>'info','failed'=>'dark'] @endphp
                <tr>
                    @if(!$isStudentView)
                    <td class="ps-3">
                        <div class="d-flex align-items-center gap-2">
                            <div class="rounded-circle bg-primary bg-opacity-10 text-primary d-flex align-items-center justify-content-center fw-bold flex-shrink-0"
                                 style="width:36px;height:36px;font-size:.75rem">
                                {{ strtoupper(substr(optional(optional($reg->student)->user)->name ?? 'S', 0, 1)) }}
                            </div>
                            <div>
                                <div class="fw-semibold" style="font-size:0.85rem">{{ optional(optional($reg->student)->user)->name ?? '—' }}</div>
                                <small class="text-muted">{{ optional($reg->student)->student_id }}</small>
                            </div>
                        </div>
                    </td>
                    @if($isLecturerView)
                    <td><small class="text-muted">{{ optional(optional($reg->student)->program)->name ?? '—' }}</small></td>
                    @endif
                    @endif
                    <td class="{{ $isStudentView ? 'ps-3' : '' }}">
                        <div class="fw-semibold" style="font-size:0.85rem">{{ optional(optional($reg->courseOffering)->course)->code }}</div>
                        <small class="text-muted">{{ Str::limit(optional(optional($reg->courseOffering)->course)->title ?? optional(optional($reg->courseOffering)->course)->name ?? '', 40) }}</small>
                    </td>
                    <td><small>{{ optional(optional($reg->courseOffering)->semester)->name ?? '—' }}</small></td>
                    @if(!$isLecturerView)
                    <td><small>{{ optional(optional(optional($reg->courseOffering)->lecturer)->user)->name ?? '—' }}</small></td>
                    @endif
                    <td><span class="badge bg-{{ $colors[$reg->status] ?? 'secondary' }}">{{ ucfirst($reg->status) }}</span></td>
                    <td><small class="text-muted">{{ $reg->created_at->format('d M Y') }}</small></td>
                    @can('manage-academic')
                    <td class="text-end pe-3">
                        @if($reg->status === 'registered')
                        <form method="POST" action="{{ route('academic.registrations.drop', $reg) }}" class="d-inline"
                ...
                <tr>
                    <td colspan="99" class="text-center text-muted py-5">
                        ...
                        @if($isStudentView) You are not registered in any courses yet.
                        @elseif($isLecturerView) No students found in your courses.
                        @else No registrations found. @endif
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="card-footer bg-transparent d-flex justify-content-between align-items-center py-2">
        <small class="text-muted">
            @if(!$isStudentView && !$isLecturerView && $studentList)
                Showing {{ $studentList->firstItem() }}–{{ $studentList->lastItem() }} of {{ $studentList->total() }} students
            @else
                Showing {{ $registrations->firstItem() }}–{{ $registrations->lastItem() }} of {{ $registrations->total() }}
                {{ $isStudentView ? 'courses' : 'registrations' }}
            @endif
        </small>
        @if(!$isStudentView && !$isLecturerView && $studentList)
...
<div class="modal fade" id="registerModal" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <form method="POST" action="{{ route('academic.registrations.register') }}" id="bulkRegForm">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
...
function viewSubjects(studentId, studentName, studentNo) {
    ...
    fetch(`{{ url('academic/registrations/students') }}/${studentId}/subjects`, {
        ...
        .then(data => {
            ...
            if (!data.length) {
                ...
                const dropBase = '{{ url("academic/registrations") }}';
...
function editRegistrations(studentId, studentName, studentNo) {
    ...
    const csrf = '{{ csrf_token() }}';
    const statusBase = '{{ url("academic/registrations") }}';
