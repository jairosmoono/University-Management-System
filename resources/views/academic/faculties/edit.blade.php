    ...
    <div class="card-body">
        <form action="{{ route('academic.faculties.update', $faculty) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="code" class="form-control @error('code') is-invalid @enderror" value="{{ old('code', $faculty->code) }}" required>
            ...
            <div class="mb-3">
                ...
                <select name="dean_id" class="form-select">
                    ...
                    @foreach($staff as $s)
                        <option value="{{ $s->id }}" {{ old('dean_id', $faculty->dean_id) == $s->id ? 'selected' : '' }}>{{ $s->full_name ?? $s->user->name }}</option>
                    @endforeach
                </select>
            </div>
